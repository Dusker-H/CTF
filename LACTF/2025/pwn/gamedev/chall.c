...
void edit_level()
{
    ...
    fgets(curr->data, 0x40, stdin); // BOF 발생 
}
...
void init()
{
    ...
    // Add starting level
    start = malloc(sizeof(struct Level)); // 1개의 힙 할당
    start->data[0] = '\0';
    for (int i = 0; i < 8; i++)
        start->next[i] = NULL; // 8개의 주소에 NULL 대입
    curr = start;
}
