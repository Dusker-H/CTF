...
app.post("/start", asyncHandler(async (req, res) => {
    ...
    if (response && response.issuer && response.authorization_endpoint && response.token_endpoint && response.userinfo_endpoint) {
        ...
        const search = new URLSearchParams(); // OpenID Connect의 Authorization Request를 구성하는 쿼리 파라미터를 설정합니다:
        ...
        res.setHeader("Set-Cookie", `session=${session}; HttpOnly; Max-Age=3600; SameSite=Lax; Secure`); // SameSite=Lax 크로스사이트 요청에 대해 쿠키를 제한
